...
(SELECT us_dollars_to_currency_ratio
FROM Currency
WHERE currency_name = 'Euro'), '€0,0.00') AS price_in_euro
FROM Product WHERE price_in_us_dollars * 
(SELECT us_dollars_to_currency_ratio FROM Currency
WHERE currency_name = 'Euro') <25 OR price_in_us_dollars * 
(SELECT us_dollars_to_currency_ratio FROM Currency 
